...
def game(N_episodes, AI_type,Intrinsic_type):
    ...
    update_timestep = 200#2000      # update policy every n timesteps
    lr = 0.002
    betas = (0.9, 0.999)
    ...
    for i_episode in range(1, max_episodes+1):
        ...
        #while not done:
            ...
            if ((AI_type == "PPO"or AI_type == "A2C") and Intrinsic_type == "1"):
                ...
                reward_sum = reward + intrinsic_rewards
            elif ((AI_type == "PPO"or AI_type == "A2C") and Intrinsic_type == "6"):
                intrinsic_rewards = get_intrinsic_rewards(AI_type,state,ppo,n_agents,100)
                #print("intrinsic_rewards = ",intrinsic_rewards)
                reward_sum = intrinsic_rewards
            else:
                reward_sum = reward + intrinsic_rewards
    ...
    return avg_rewards, best_avg_reward,samp_rewards,"0",state_mem
...
def main():
    ...
    for AI in AI1:
        ...
        """
        avg_reward, best_avg_reward,samp_rewards,stats,state_mem = game(N_episodes, AI, "6") 
        plot(name,avg_reward, best_avg_reward,samp_rewards, AI,'r')
        plot_state_record(state_mem,name,AI)
        unique, counts = np.unique(state_mem, return_counts=True)
        print(dict(zip(unique, counts)))
        print(unique)
        print("total unique number =", len(unique))
        print("total reward =",np.sum(samp_rewards))
        """
        
        avg_reward, best_avg_reward,samp_rewards,stats,state_mem = game(N_episodes, AI,"0") 
        plot(name,avg_reward, best_avg_reward,samp_rewards, AI,'b')
        #plot_state_record(state_mem,name,AI)
        #unique, counts = np.unique(state_mem, return_counts=True)
        #print(dict(zip(unique, counts)))
        #print(unique)
        ...
        """
        avg_reward, best_avg_reward,samp_rewards,stats,state_mem = game(N_episodes, AI, "1") 
        plot(name,avg_reward, best_avg_reward,samp_rewards, AI,'g')
        plot_state_record(state_mem,name,AI)
        unique, counts = np.unique(state_mem, return_counts=True)
        print(dict(zip(unique, counts)))
        print(unique)
        ...
        plot(name,avg_reward, best_avg_reward,samp_rewards, AI,'c')
        avg_reward, best_avg_reward,samp_rewards,stats = game(N_episodes, AI, "4") 
        plot(name,avg_reward, best_avg_reward,samp_rewards, AI,'m')
        avg_reward, best_avg_reward,samp_rewards,stats = game(N_episodes, AI, "5") 
        plot(name,avg_reward, best_avg_reward,samp_rewards, AI,'y')
        """
...
def plot_state_record(state_mem,name,AI):
    plt.figure('State_Memory_distribution'+ AI+' in' + name)
    plt.hist(state_mem)
    plt.xlabel('State')
    plt.ylabel('Counts of ' + AI)
    plt.title(AI)
    plt.show()
