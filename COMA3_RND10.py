...
class COMA(object):
    ...
    def train(self, o1_list, a1_list, pi_a1_list, o2_list, a2_list, pi_a2_list, r_list,adv1_int,adv2_int):
        ...
        for t in range(T):
            #print(adv1_int)
            a1_loss = a1_loss + (A1_list[t].item()+adv1_int[t][0])* torch.log(pi_a1_list[t][0, a1_list[t]])
        a1_loss = -a1_loss / T 
        #print(adv1_int[t][0]/A1_list[t].item())
        ...
        for t in range(T):
            a2_loss = a2_loss + (A2_list[t].item()+adv2_int[t][0]) * torch.log(pi_a2_list[t][0, a2_list[t]])
        a2_loss = -a2_loss / T 
...
def game(mode):
    ...
    for epi_iter in range(max_epi_iter):
        ...
        if mode == 'intrinsic':
            adv1_int = (r1_int_list)/np.sqrt(reward_rms1.var)#total_int_rwd1_list - reward_rms1.mean/np.sqrt(reward_rms1.var)
            adv2_int = (r2_int_list)/np.sqrt(reward_rms2.var)#total_int_rwd2_list - reward_rms2.mean/np.sqrt(reward_rms2.var)
