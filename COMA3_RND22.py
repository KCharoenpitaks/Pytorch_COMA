...
from env_FindGoals_sparse_rewards import EnvFindGoals_sparse
...
class RNDModelAll(nn.Module):
    def __init__(self, output_size=64):
        super(RNDModelAll, self).__init__()
        self.output_size = output_size
        self.conv1_tar = nn.Conv2d(in_channels=3, out_channels=16, kernel_size=3, stride=1)
        self.flat1_tar = Flatten()
        self.fc1_tar = nn.Linear(120, 64)
        self.fc2_tar = nn.Linear(64, output_size)
        self.conv1_pred = nn.Conv2d(in_channels=3, out_channels=16, kernel_size=3, stride=1)
        self.flat1_pred = Flatten()
        self.fc1_pred = nn.Linear(120, 64)
        self.fc2_pred = nn.Linear(64, output_size)
    ...
    def target_forward_(self, next_obs):
        #target_feature = self.target(next_obs)
        #predict_feature = self.predictor(next_obs)
        #h1 = F.relu(self.conv1_tar(next_obs))
        #print("next_obs",next_obs.shape)
        #print("next_obs",next_obs.reshape(-1,1).shape)
        #h1 = self.flat1_tar(next_obs.reshape(1,-1))
        x = F.relu(self.fc1_tar(next_obs.reshape(1,-1,120)))
        x = self.fc2_tar(x)
        return x
    
    def predictor_forward_(self, next_obs):
        #target_feature = self.target(next_obs)
        #predict_feature = self.predictor(next_obs)
        #h1 = F.relu(self.conv1_pred(next_obs))
        #h1 = self.flat1_pred(next_obs.reshape(1,-1))
        x = F.relu(self.fc1_pred(next_obs.reshape(1,-1,120)))
        x = self.fc2_pred(x)
        return x
...
class State_prediction(nn.Module):
    ...
    def forward_all(self,obs,action1,action2):
        ...
        h_all2 = torch.cat((h_all,action1,action2),1)
...
class COMA(object):
    ...
    def compute_intrinsic_reward1(self, obs):
        ...
        #print("obs rwd1.shape",self.img_to_tensor(obs).unsqueeze(0).shape)
    ...
    def compute_intrinsic_all(self, obs):
        obs = torch.FloatTensor(obs).to(self.device)
        #print("obs.shape",obs.shape)
        #print("obs.shape",self.img_to_tensor(obs).unsqueeze(0).shape)
        target_next_feature = self.rndall.target_forward_(self.img_to_tensor(obs).unsqueeze(0))
        predict_next_feature = self.rndall.predictor_forward_(self.img_to_tensor(obs).unsqueeze(0))
        intrinsic_reward = (target_next_feature - predict_next_feature).pow(2).sum(1) / 2
    ...
    def train(self, o1_list, a1_list, pi_a1_list, o2_list, a2_list, pi_a2_list, r_list,adv1_int,adv2_int,pred_s_list,pred_s_dat_list,adv_sur_int,clip_ratio,obsall_list):
        ...
        rndall_optimizer = torch.optim.Adam(self.rndall.parameters(), lr=1e-3)
        ...
        #o1_list = torch.from_numpy(np.array(o1_list))
        #o2_list = torch.from_numpy(np.array(o2_list))
        #obs_for_rndall = torch.from_numpy(np.array(obsall_list))
        #self.img_to_tensor(o1_list).unsqueeze(0)
        """
        obs_for_rndall_list = []
        obs_for_rnd2_list = []
        obs_for_rnd1_list = []
        for i in range (len(o1_list)):
            obs_for_rnd1 = self.img_to_tensor(o1_list[i]).unsqueeze(0)
            obs_for_rnd2 = self.img_to_tensor(o2_list[i]).unsqueeze(0)
            obs_for_rndall = self.img_to_tensor(obsall_list[i]).unsqueeze(0)
            obs_for_rndall_list.append(np.array(obs_for_rnd1_list))
            obs_for_rnd1_list.append(np.array(obs_for_rnd1))
            obs_for_rnd2_list.append(np.array(obs_for_rnd2))
        print(obs_for_rndall_list)
        """
        
        #obs_for_rndall_list = torch.from_numpy(obs_for_rndall_list)
        #obs_for_rnd1_list = torch.from_numpy(obs_for_rnd1_list)
        #obs_for_rnd2_list = torch.from_numpy(obs_for_rnd2_list)

        
        #print(obs_for_rnd2_list)
        #print("------------")
        #print(obs_for_rndall_list)
        ...
        for t in range(T):
            #print(A1_list[t].item())
            adv1_int_scaled = np.clip(adv1_int[t][0],clip_ratio*-A1_list[t].item(),clip_ratio*A1_list[t].item())#/(np.power(10,magnitude(adv1_int[t]*10)-magnitude(A1_list[t])))
            adv1_sur_int_scaled = np.clip(adv_sur_int[t][0],clip_ratio*-A1_list[t].item(),clip_ratio*A1_list[t].item())
            #adv1_sur_int_scaled = np.clip(adv_sur_int[t][0],-0.4,0.4)#adv_sur_int[t][0]/(np.power(10,magnitude(adv_sur_int[t]*10)-magnitude(A1_list[t])))
            
            
            a1_loss = a1_loss + (A1_list[t].item()+adv1_int_scaled+adv1_sur_int_scaled)* torch.log(pi_a1_list[t][0, a1_list[t]])
        ...
        for t in range(T):
            ...
            adv2_int_scaled =  np.clip(adv2_int[t][0],clip_ratio*-A1_list[t].item(),clip_ratio*A1_list[t].item())#/(np.power(10,magnitude(adv2_int[t]*10)-magnitude(A2_list[t])))
            adv2_sur_int_scaled =  np.clip(adv_sur_int[t][0],clip_ratio*-A1_list[t].item(),clip_ratio*A1_list[t].item())#/(np.power(10,magnitude(adv2_int[t]*10)-magnitude(A2_list[t])))
            #adv2_sur_int_scaled = np.clip(adv_sur_int[t][0],-0.4,0.4)#adv_sur_int[t][0]/(np.power(10,magnitude(adv_sur_int[t]*10)-magnitude(A2_list[t])))
            
            a2_loss = a2_loss + (A2_list[t].item()+adv2_int_scaled+adv2_sur_int_scaled) * torch.log(pi_a2_list[t][0, a2_list[t]])
        ...
        r1_loss = self.c_loss_fn(self.rnd1.target_forward(o1_list.float()).detach(),self.rnd1.predictor_forward(o1_list.float()))
        rnd1_optimizer.zero_grad()
        r1_loss.sum().backward()
        rnd1_optimizer.step()
        
        r2_loss = self.c_loss_fn(self.rnd2.target_forward(o2_list.float()).detach(),self.rnd2.predictor_forward(o2_list.float()))
        rnd2_optimizer.zero_grad()
        r2_loss.sum().backward()
        rnd2_optimizer.step()
        
        #print("obs_for_rndall",obs_for_rndall.shape)

        rall_loss = self.c_loss_fn(self.rndall.target_forward_(obs_for_rndall.float()).detach(),self.rndall.predictor_forward_(obs_for_rndall.float()))
        #print("rall_loss",rall_loss)
        rndall_optimizer.zero_grad()
        rall_loss.sum().backward()
        rndall_optimizer.step()
    
def game(mode, clip_ratio):
    torch.set_num_threads(1)
    env = EnvFindGoals_sparse()
    max_epi_iter = 2101#2101
    max_MC_iter = 200
    N_action = 5
    ...
    for epi_iter in range(max_epi_iter):
        ...
        for MC_iter in range(max_MC_iter):
            ...
            obsall_list.append(obs_all_s)
            #print("obs_all_s =",obs_all_s)
            #print("intrinsic reward =",agent.compute_intrinsic_reward1(obs1))
            #print(((obs1-obs1_norm.mean)/(obs1_norm.var+0.0001)).clip(-5, 5))
            action1, pi_a1, action2, pi_a2,v1,v2 = agent.get_action(((obs1-obs1_norm.mean)/(np.sqrt(obs1_norm.var)+0.00001)).clip(-5, 5), ((obs2-obs2_norm.mean)/(np.sqrt(obs2_norm.var)+0.00001)).clip(-5, 5))
            ...
            int_rwd1 = agent.compute_intrinsic_reward1(((obs1-obs1_norm.mean)/np.sqrt(obs1_norm.var)).clip(-5, 5))
            int_rwd2 = agent.compute_intrinsic_reward2(((obs2-obs2_norm.mean)/np.sqrt(obs2_norm.var)).clip(-5, 5))
            int_all = agent.compute_intrinsic_all(((obs_all_s-obsall.mean)/np.sqrt(obsall.var)).clip(-5, 5))
            #print("int_all=",int_all.sum())
            ...
            rall_int_list.append([int_all.sum()])
        ...
        meanall, stdall, countall = np.mean(rall_int_list), np.std(rall_int_list), len(rall_int_list)
        ...
        reward_rmsall.update_from_moments(meanall, stdall ** 2, countall)
        ...
        total_int_rwdall_list = rall_int_list/np.sqrt(reward_rmsall.var)
        

        dones_list = np.stack(np.expand_dims(dones_list,1)).transpose()
        total_int_rwd1_list = np.stack(total_int_rwd1_list).transpose()
        total_int_rwd2_list = np.stack(total_int_rwd2_list).transpose()
        total_int_rwdall_list = np.stack(total_int_rwdall_list).transpose()
        ...
        if mode == 'intrinsic':
            ...
            adv1_int = (r1_int_list)/np.sqrt(reward_rms1.var)
            #np.zeros((len(r1_int_list),1))#(r1_int_list)/np.sqrt(reward_rms1.var)
            #total_int_rwd1_list - reward_rms1.mean/np.sqrt(reward_rms1.var)
            adv2_int = (r2_int_list)/np.sqrt(reward_rms2.var)
            #np.zeros((len(r1_int_list),1))#(r2_int_list)/np.sqrt(reward_rms2.var)
            #total_int_rwd2_list - reward_rms2.mean/np.sqrt(reward_rms2.var)
            adv_sur_int = np.zeros((len(loss_intrisic_surprise_list),1))#loss_intrisic_surprise_list/np.sqrt(loss_intrisic_rms.var)#np.zeros((len(loss_intrisic_surprise_list),1))#loss_intrisic_surprise_list/np.sqrt(loss_intrisic_rms.var)
        
        elif mode == 'combined':
            
            adv1_int = (rall_int_list)/(np.sqrt(reward_rmsall.var)*2)
            adv2_int = (rall_int_list)/(np.sqrt(reward_rmsall.var)*2)
            adv_sur_int = np.zeros((len(loss_intrisic_surprise_list),1))
        ...
        print("####################################")
        agent.train(o1_list, a1_list, pi_a1_list, o2_list, a2_list, pi_a2_list, r_list,adv1_int,adv2_int,pred_s_list,pred_s_dat_list,adv_sur_int, clip_ratio,obsall_list)
...
def plot_sum(name,mean, std,color): #def plot(name,time, Average_win,color):
    ...
    plt.fill_between(t,mean-std,mean+std,alpha=0.3,linewidth=0)
    plt.xlabel('Episode')
    plt.ylabel('Rewards')
...
if __name__ == '__main__':
    loop = 5
    #plot= {}
    modes = [('combined',1),('combined',2),('intrinsic',0.5),('intrinsic',1),('intrinsic',2),('normal',0)]
    ...
    collect_data_temp_int2 = []
    collect_data_temp_avg_int_ma2 = []
    collect_data_temp_std_int_ma2 = []
    collect_data_temp_int5 = []
    collect_data_temp_avg_int_ma5 = []
    collect_data_temp_std_int_ma5 = []
    collect_data_temp_int_1 = []
    collect_data_temp_avg_int_ma_1 = []
    collect_data_temp_std_int_ma_1 = []
    collect_data_temp_int_2 = []
    collect_data_temp_avg_int_ma_2 = []
    collect_data_temp_std_int_ma_2 = []
    for (mode, clip_ratio) in modes:
        round_data = {}
        for i in range(loop):
            train_curve = game(mode, clip_ratio)
            round_data[i]=train_curve
            if mode =='combined'and clip_ratio == 1:
                color = 'r'
            elif mode =='combined'and clip_ratio == 2:
                color = 'c'
            elif mode =='intrinsic'and clip_ratio == 0.5:
                color = 'b'
            elif mode =='intrinsic'and clip_ratio == 1:
                color = 'm'
            elif mode =='intrinsic'and clip_ratio == 2:
                color = 'k'
            elif mode == 'normal':
                color = 'g'
            else:
        ...
        mode_data[mode+"_"+str(clip_ratio)] = round_data
        """
    ...
    ################# 
    for i in range (loop):
        collect_data_temp_int.append(mode_data[modes[0][0]+"_"+str(modes[0][1])][i])
    train_curve_avg_int = np.mean(collect_data_temp_int,0)
    train_curve_std_int = np.std(collect_data_temp_int,0)
    ...
    collect_data_temp_avg_int_ma_0_1 = np.array(collect_data_temp_avg_int_ma)
    collect_data_temp_std_int_ma_0_1 = np.array(collect_data_temp_std_int_ma)
    ...
    ################# 
    for i in range (loop):
        collect_data_temp_int2.append(mode_data[modes[1][0]+"_"+str(modes[1][1])][i])
    train_curve_avg_int2 = np.mean(collect_data_temp_int2,0)
    train_curve_std_int2 = np.std(collect_data_temp_int2,0)
    
    for i in range (len(train_curve_avg_int2)):
        if (i >= 100): 
            temp_avg_int_ma2 = np.mean(train_curve_avg_int2[i-100:i])
            collect_data_temp_avg_int_ma2.append(temp_avg_int_ma2)
            temp_std_int_ma2 = np.mean(train_curve_std_int2[i-100:i])
            collect_data_temp_std_int_ma2.append(temp_std_int_ma2)
    collect_data_temp_avg_int_ma_0_2 = np.array(collect_data_temp_avg_int_ma2)
    collect_data_temp_std_int_ma_0_2 = np.array(collect_data_temp_std_int_ma2)
    #train_curve_avg_int_ma = np.mean(collect_data_temp_int_ma,axis =0)
    #train_curve_std_int_ma = np.std(collect_data_temp_int_ma,axis =0)
    ...
    ################# 
    for i in range (loop):
        collect_data_temp_int5.append(mode_data[modes[2][0]+"_"+str(modes[2][1])][i])
    train_curve_avg_int5 = np.mean(collect_data_temp_int5,0)
    train_curve_std_int5 = np.std(collect_data_temp_int5,0)
    
    for i in range (len(train_curve_avg_int5)):
        if (i >= 100): 
            temp_avg_int_ma5 = np.mean(train_curve_avg_int5[i-100:i])
            collect_data_temp_avg_int_ma5.append(temp_avg_int_ma5)
            temp_std_int_ma5 = np.mean(train_curve_std_int5[i-100:i])
            collect_data_temp_std_int_ma5.append(temp_std_int_ma5)
    collect_data_temp_avg_int_ma_0_5 = np.array(collect_data_temp_avg_int_ma5)
    collect_data_temp_std_int_ma_0_5 = np.array(collect_data_temp_std_int_ma5)
    #train_curve_avg_int_ma = np.mean(collect_data_temp_int_ma,axis =0)
    #train_curve_std_int_ma = np.std(collect_data_temp_int_ma,axis =0)
    ...
    ################# 
    for i in range (loop):
        collect_data_temp_int_1.append(mode_data[modes[3][0]+"_"+str(modes[3][1])][i])
    train_curve_avg_int_1 = np.mean(collect_data_temp_int_1,0)
    train_curve_std_int_1 = np.std(collect_data_temp_int_1,0)
    
    for i in range (len(train_curve_avg_int_1)):
        if (i >= 100): 
            temp_avg_int_ma_1 = np.mean(train_curve_avg_int_1[i-100:i])
            collect_data_temp_avg_int_ma_1.append(temp_avg_int_ma_1)
            temp_std_int_ma_1 = np.mean(train_curve_std_int_1[i-100:i])
            collect_data_temp_std_int_ma_1.append(temp_std_int_ma_1)
    collect_data_temp_avg_int_ma_1_0 = np.array(collect_data_temp_avg_int_ma_1)
    collect_data_temp_std_int_ma_1_0 = np.array(collect_data_temp_std_int_ma_1)
    #train_curve_avg_int_ma = np.mean(collect_data_temp_int_ma,axis =0)
    #train_curve_std_int_ma = np.std(collect_data_temp_int_ma,axis =0)
    ...
    for i in range (loop):
        collect_data_temp_int_2.append(mode_data[modes[4][0]+"_"+str(modes[4][1])][i])
    train_curve_avg_int_2 = np.mean(collect_data_temp_int_2,0)
    train_curve_std_int_2 = np.std(collect_data_temp_int_2,0)   

    for i in range (len(train_curve_avg_int_2)):
        if (i >= 100):
            temp_avg_int_ma_2 = np.mean(train_curve_avg_int_2[i-100:i])
            collect_data_temp_avg_int_ma_2.append(temp_avg_int_ma_2)
            temp_std_int_ma_2 = np.mean(train_curve_std_int_2[i-100:i])
            collect_data_temp_std_int_ma_2.append(temp_std_int_ma_2)
    collect_data_temp_avg_int_ma_2_0 = np.array(collect_data_temp_avg_int_ma_2)
    collect_data_temp_std_int_ma_2_0 = np.array(collect_data_temp_std_int_ma_2)
    ##############
    ################
    
    for i in range (loop):
        collect_data_temp_norm.append(mode_data[modes[5][0]+"_"+str(modes[5][1])][i])
    train_curve_avg_norm = np.mean(collect_data_temp_norm,0)
    train_curve_std_norm = np.std(collect_data_temp_norm,0)   
    ...
    plot_sum("MARL_RND_COMA_1.0_clip_ratio_com",collect_data_temp_avg_int_ma_0_1,collect_data_temp_std_int_ma_0_1,color)
    plot_sum("MARL_RND_COMA_2.0_clip_ratio_com",collect_data_temp_avg_int_ma_0_2,collect_data_temp_std_int_ma_0_2,color)
    plot_sum("MARL_RND_COMA_0.5_clip_ratio",collect_data_temp_avg_int_ma_0_5,collect_data_temp_std_int_ma_0_5,color)
    plot_sum("MARL_RND_COMA_1.0_clip_ratio",collect_data_temp_avg_int_ma_1_0,collect_data_temp_std_int_ma_1_0,color)
    plot_sum("MARL_RND_COMA_2.0_clip_ratio",collect_data_temp_avg_int_ma_2_0,collect_data_temp_std_int_ma_2_0,color)
