...
class PPO:
    ...
    def update(self, memory,adv_int,clip_ratio):   
        ...
        for _ in range(self.K_epochs):
            ...
            #print("torch.tensor(adv_int).float()=",torch.tensor(adv_int).float())
            #print("-0---", torch.clamp(torch.tensor(adv_int).float(),torch.max(0.6*advantages, torch.tensor(adv_int).float()),torch.min(1.4*advantages, torch.tensor(adv_int).float())))
            #torch.clamp(torch.tensor(adv_int).float(),-0.4*advantages,0.4*advantages)
            #surr1 = ratios * (advantages + 0.1*torch.tensor(adv_int).float())
            #print(advantages)
            #print(torch.clamp(torch.tensor(adv_int).float(),-0.4,0.4))
            surr1 = 0
            surr2 = 0
            for i in range (len(advantages)):
                surr1 += ratios[i]*(advantages[i] +clip_ratio* torch.clamp(torch.tensor(adv_int[i]).float(),-advantages[i],advantages[i]))
                surr2 += torch.clamp(ratios[i], 1-self.eps_clip, 1+self.eps_clip) *(advantages[i] + clip_ratio*torch.clamp(torch.tensor(adv_int[i]).float(),-advantages[i],advantages[i]))

                
                
            #surr1 = ratios * (advantages + torch.clamp(torch.tensor(adv_int).float(),-0.1,0.1))
            #surr2 = torch.clamp(ratios, 1-self.eps_clip, 1+self.eps_clip) * (advantages + torch.clamp(torch.tensor(adv_int).float(),-0.1,0.1))
...
def game(N_episodes, AI_type,Intrinsic_type, clip_ratio):
    ...
    for i_episode in range(1, max_episodes+1):
        ...
        while not done:
            ...
            # Running policy_old:
            state_norm = (state - obs1_norm.mean)/np.sqrt(obs1_norm.var)
            action = ppo.policy_old.act(state_norm, memory)
            ...
            if timestep % update_timestep == 0:
                ...
                mean1, std1, count1 = np.mean(temp_int), np.std(temp_int), len(temp_int)
                reward_rms.update_from_moments(mean1, std1 ** 2, count1)
                obs1_norm.update(temp_state)
                adv_int = (temp_int-reward_rms.mean)/np.sqrt(reward_rms.var)
...
def main():
    ...
    for AI in AI1:
        ...
        plot("LunarLander",avg_reward, best_avg_reward,samp_rewards, AI,'b')
        avg_reward, best_avg_reward,samp_rewards,stats = game(N_episodes, AI, "1",0.1) 
        plot("LunarLander",avg_reward, best_avg_reward,samp_rewards, AI,'g')
        avg_reward, best_avg_reward,samp_rewards,stats = game(N_episodes, AI, "1",0.2) 
        plot("LunarLander",avg_reward, best_avg_reward,samp_rewards, AI,'r')
        avg_reward, best_avg_reward,samp_rewards,stats = game(N_episodes, AI, "1",0.5) 
        plot("LunarLander",avg_reward, best_avg_reward,samp_rewards, AI,'y')
        avg_reward, best_avg_reward,samp_rewards,stats = game(N_episodes, AI, "1",1) 
        plot("LunarLander",avg_reward, best_avg_reward,samp_rewards, AI,'p')
        """
        ...
        plot("LunarLander",avg_reward, best_avg_reward,samp_rewards, AI,'c')
        avg_reward, best_avg_reward,samp_rewards,stats = game(N_episodes, AI, "4") 
        plot("LunarLander",avg_reward, best_avg_reward,samp_rewards, AI,'m')
